...
WRITE_NETDATA_HTPASSWD=0
if [ -n "${NETDATA_WEB_PASSWORD:-}" ]; then
  WRITE_NETDATA_HTPASSWD=1
...
if [ "$WRITE_NETDATA_HTPASSWD" -eq 1 ]; then
  ...
  echo "   → Keeping existing nginx Basic Auth credentials at $NETDATA_HTPASSWD"
fi
